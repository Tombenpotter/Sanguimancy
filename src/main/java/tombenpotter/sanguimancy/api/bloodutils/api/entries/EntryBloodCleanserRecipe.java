...
public class EntryBloodCleanserRecipe extends EntryBase {

    public RecipeBloodCleanser recipe;
    public ItemStack input, output;
    public int time, bloodRequired;
    public ArrayList<ItemIcon> icons = new ArrayList<ItemIcon>();

    public EntryBloodCleanserRecipe(RecipeBloodCleanser recipe) {
        this.recipe = recipe;
        populate(recipe);
    }

    public void populate(RecipeBloodCleanser recipe) {
        this.input = recipe.fInput;
        this.output = recipe.fOutput;
        this.time = 200;
        this.bloodRequired = 1000;
    }
    ...
    public void draw(GuiEntry entry, int width, int height, int left, int top, EntityPlayer player, String key, int page, int mX, int mY) {
        Minecraft.getMinecraft().fontRenderer.drawSplitString(StatCollector.translateToLocal("compat.nei.corrupted.infusion.time") + ": " + String.valueOf(time) + " ticks", left + width / 2 - 58, top + 15, 110, 0);
        Minecraft.getMinecraft().fontRenderer.drawSplitString(StatCollector.translateToLocal("compat.nei.blood.required") + ": " + String.valueOf(bloodRequired) + "mB", left + width / 2 - 58, top + 35, 110, 0);
