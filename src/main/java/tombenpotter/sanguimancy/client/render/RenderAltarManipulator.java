...
public class RenderAltarManipulator extends TileEntitySpecialRenderer<TileAltarManipulator> {
    ...
    @Override
    public void render(TileAltarManipulator tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        renderModel(tile, x, y, z);
        GL11.glPushMatrix();
        if (!tile.getInventory(null).getStackInSlot(1).isEmpty()) {
        	ItemStack displayItem = tile.getInventory(null).getStackInSlot(1);
